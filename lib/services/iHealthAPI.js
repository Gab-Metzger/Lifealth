...
function _getDatasForApi(api, apiListKey, user, accessToken, from, to, resolve, reject) {
  var fromTime = (from)?from.getTime():null;
  var toTime = (to)?to.getTime():null;
  _getDatas(api, user, accessToken, 1, fromTime, toTime, function (data) {
    var promises = [];
    ...
    if (pageCount > 0) {
      ...
      for (var i = 1; i < pageCount + 1; i++) {
        promises.push(new Promise(function (resolvep, rejectp) {
          _getDatas(api, user, accessToken, i + 1, fromTime, toTime, function (data) {
