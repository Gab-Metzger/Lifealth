var util = require('util'), querystring = require('querystring'), url = require('url'), format = require('string-format'), OAuth2Strategy = require('passport-oauth').OAuth2Strategy, InternalOAuthError = require('passport-oauth').InternalOAuthError,
...
Strategy.prototype.userProfile = function (accessToken, done) {
  ...
  this._oauth2.get(url, accessToken, function (err, body, res) {
    ...
    try {
      ...
      if (json.Error) {
        return done(new InternalOAuthError(json.ErrorDescription, err));
      }
  ...
  if (req.query && req.query.code) {
    ...
    this._oauth2._request("GET", accessTokenUrl, post_headers, null, null, function (error, data, response) {
      if (error)  callback(error);
        ...
        if (params.ErrorCode) {
          callback(params.ErrorDescription);
        } else {
