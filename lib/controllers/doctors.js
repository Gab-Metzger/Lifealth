...
exports.removeInvite = function (req, res) {
  User.findById(req.params.inviteId, function (err, user) {
    if (err) res.send(500, err);
    else if (user) {
      User.findByIdAndUpdate(req.params.doctorId, {$pull: {links: {email: user.email}}}, {safe: true}, function (err, doctor) {
        console.log('remove email ' + user.email + ' from links ' + doctor.links);
        res.send(200);
      });
      if (user.links.length == 0) {
        user.remove(function (err, user) {
          console.log('remove user patient');
        });
      }
    } else res.send(404);
  });
    ...
    if (err) res.send(500, err);
      ...
      User.findById(req.params.doctorId, function (err, doctor) {
        if (err) res.send(500, err)
          ...
          console.log('resend email ' + user.email + ' from ' + doctor.links);
          mailer.sendMail(user, doctor, req.body.notes);
          res.send(200);
        } else res.send(404);
      });
    } else res.send(404);
