        ...
        if (user.iHealth && user.iHealth.userid) {
          iHealthAPI.getBPDatas(user, user.accessToken, req.query.from, req.query.to, function (data) {
            if (user.bpDatas.length == 0) {
              ...
              res.send(data.concat(user.bpDatasInRange(req.query.from, req.query.to)).sort(sortByMDate));
            }
          }, function (err) {
            res.send(user.bpDatasInRange(req.query.from, req.query.to).sort(sortByMDate));
          });
        } else {
          res.send(user.bpDatasInRange(req.query.from, req.query.to).sort(sortByMDate));
        }
        ...
        if (user.iHealth && user.iHealth.userid) {
          iHealthAPI.getBGDatas(user, user.accessToken, req.query.from, req.query.to, function (data) {
            if (user.bgDatas.length == 0) {
              ...
              res.send(data.concat(user.bgDatasInRange(req.query.from, req.query.to)).sort(sortByMDate));
            }
          }, function (err) {
            res.send(user.bgDatasInRange(req.query.from, req.query.to).sort(sortByMDate));
          });
        } else {
          res.send(user.bgDatasInRange(req.query.from, req.query.to).sort(sortByMDate));
        }
